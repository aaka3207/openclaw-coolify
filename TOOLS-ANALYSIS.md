...
```bash
# SSH into container
ssh netcup "docker exec openclaw-qsw0sgsgwcog4wg88g448sgs-064241424802 bash"
...
```bash
# Daily health check
ssh netcup "docker exec openclaw-qsw0sgsgwcog4wg88g448sgs-064241424802 openclaw cron add --name health-check --schedule '0 2 * * *' --message 'Run health check: container status, memory usage, error logs, sandbox count. Report if issues found.'"

# Weekly security audit
ssh netcup "docker exec openclaw-qsw0sgsgwcog4wg88g448sgs-064241424802 openclaw cron add --name security-audit --schedule '0 3 * * 0' --message 'Run security audit --deep and report any warnings or critical issues.'"
```
...
ssh netcup "docker exec <container> openclaw status --deep"

# Install a skill
ssh netcup "docker exec <container> clawhub install github"

# List installed skills
ssh netcup "docker exec <container> openclaw skills list"

# Add cron job
ssh netcup "docker exec <container> openclaw cron add --name test --schedule '*/5 * * * *' --message 'Test job'"

# List cron jobs
ssh netcup "docker exec <container> openclaw cron list"

# Test web search (after API key configured)
ssh netcup "docker exec <container> openclaw message send --channel telegram --target <your-id> --message 'Search for Docker security best practices'"
```
