...
4. `grep -c 'TEMP:.*remove when CHANGELOG' scripts/bootstrap.sh` — should return 0
  ...
  <done>
bootstrap.sh starts tailscaled before openclaw gateway, patches openclaw.json to bind=loopback + tailscale.mode=serve, removes all 4 temp patches (mode=remote, remote.url, remote.token, --allow-unconfigured), and preserves useAccessGroups=false and all other existing patches.
  </done>
</task>
...
3. Removes the --port flag (Tailscale Serve uses standard HTTPS port 443)
4. Adds a connectivity check before attempting connection
...
- The `openclaw node run` command uses `--url` instead of `--host`/`--port` since it's now HTTPS
...
# 2. Start node host in background
...
# 3. Wait for node to connect and relay to start
sleep 3

# 4. Start browser relay (one-shot — starts relay, exits OK if tab not yet attached)
...
NOTE: The `openclaw node run --url` flag is the correct way to connect to a Tailscale-served gateway over HTTPS. The old `--host`/`--port` flags were for direct TCP connection which is no longer applicable.

If `--url` is not a valid flag for `openclaw node run`, fall back to using `--host` with the MagicDNS hostname and `--port 443` with `--tls`:
```bash
openclaw node run \
  --host "openclaw-server.TAILNET.ts.net" \
  --port 443 \
  --tls \
  --node-id my-macbook \
  --display-name "MacBook Pro" &
```
Check `openclaw node run --help` to determine the correct flags.
  </action>
  <verify>
...
2. `grep -n '192.168.1.100' scripts/connect-mac-node.sh` — should return ZERO results (LAN IP removed)
  ...
  <done>
connect-mac-node.sh uses Tailscale MagicDNS HTTPS URL instead of LAN IP. Includes CHANGE-ME guard so user is prompted to set their tailnet name. Verifies Tailscale is running before connecting.
  </done>
</task>
...
- connect-mac-node.sh uses Tailscale MagicDNS URL
