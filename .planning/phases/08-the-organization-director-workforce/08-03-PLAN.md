...
<task type="auto">
  ...
  <files></files>
  <action>
The Global Error Workflow setting in n8n may need to be configured via the n8n API settings endpoint or via environment variable.
...
If the settings API approach fails, the global error workflow can be set via n8n environment variable. Check if n8n is running in Docker on the server:
...
If n8n is in Docker, the env var `N8N_DEFAULT_ERROR_WORKFLOW` can be set. However, the simplest approach is to set it in the n8n UI: Settings (gear icon) > Workflows > Default Error Workflow > select "Global Error Handler -> Automation Supervisor".

If the API approach works, great. If not, create a note in the summary that the user needs to set this in n8n UI.
...
If sessions exist for automation-supervisor, the hook routed correctly.
  ...
  <done>n8n Global Error Handler workflow is active and set as the default error workflow. Test failure confirms errors route to automation-supervisor hook session.</done>
</task>
