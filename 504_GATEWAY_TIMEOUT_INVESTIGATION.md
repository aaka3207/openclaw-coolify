...
After every deployment, accessing `https://bot.appautomation.cloud` results in a **504 Gateway Timeout** error. The issue is resolved by restarting the Coolify proxy (`coolify-proxy` container), but this is not a sustainable solution.
...
1. **Container is healthy:**
   ```
   openclaw-qsw0sgsgwcog4wg88g448sgs-224425322381  Up 2 minutes (healthy)  18789/tcp
   ```
...
3. **Test WITHOUT restarting proxy:**
   ...
   curl -I https://bot.appautomation.cloud
   ```

4. **Verify Traefik sees the service:**
   ```bash
   ssh netcup "docker inspect openclaw-* | grep 'traefik.http.services'"
   ```

5. **Check logs for errors:**
   ```bash
   ssh netcup "docker logs coolify-proxy 2>&1 | tail -50"
   ```
