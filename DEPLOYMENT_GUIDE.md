...
ssh netcup "docker ps --filter name=openclaw-qsw --format '{{.Names}}'"

# 2. Backup current config
ssh netcup "docker exec <container-name> cp /root/.openclaw/openclaw.json /root/.openclaw/openclaw.json.backup-before-optimization-$(date +%Y%m%d)"

# 3. Verify volumes exist
ssh netcup "docker volume ls | grep openclaw"
# Expected: openclaw-config, openclaw-workspace, searxng-data

# 4. Check OAuth accounts
ssh netcup "docker exec <container-name> openclaw models status | grep -c 'google-antigravity:'"
# Expected: 9

# 5. Check skills
ssh netcup "docker exec <container-name> ls /root/openclaw-workspace/skills/"
# Expected: sandbox-manager, web-utils, memory-setup

# 6. Check plugins
ssh netcup "docker exec <container-name> openclaw plugins list | grep telegram-enhanced"
...
ssh netcup "docker ps --filter name=openclaw-qsw"
...
$CONTAINER = ssh netcup "docker ps --filter name=openclaw-qsw --format '{{.Names}}' | head -1"

# Run test script
ssh netcup "docker exec $CONTAINER bash /app/scripts/test-deployment.sh"
```

**Expected Output:**
```
🧪 Testing OpenClaw deployment: openclaw-qsw0sgsgwcog4wg88g448sgs-...
...
ssh netcup "docker exec $CONTAINER openclaw models status"
# Should show all 9 google-antigravity accounts

# 2. Check sandbox network (should be dynamic now)
ssh netcup "docker exec $CONTAINER openclaw config get agents.defaults.sandbox.docker.network"
# Should show: qsw0sgsgwcog4wg88g448sgs_openclaw-internal (or similar)

# 3. Test Telegram bot
# Send message to @meinopenclawbot
# Expected: Bot responds normally

# 4. Check dashboard access
# Open: https://bot.appautomation.cloud?token=<your-token>
# Expected: Dashboard loads, shows agent status

# 5. Verify new home volume created
ssh netcup "docker volume ls | grep openclaw-home"
# Expected: openclaw-home volume exists

# 6. Check Playwright browsers cached
ssh netcup "docker exec $CONTAINER ls -lh /home/node/.cache/ms-playwright/"
...
```powershell
# 1. Restart container
ssh netcup "docker restart $CONTAINER"
...
```powershell
# 1. List backups
ssh netcup "docker exec $CONTAINER ls -lh /root/.openclaw/*.backup*"

# 2. Restore specific backup
ssh netcup "docker exec $CONTAINER cp /root/.openclaw/openclaw.json.backup-before-optimization-YYYYMMDD /root/.openclaw/openclaw.json"
...
ssh netcup "docker exec $CONTAINER cat /root/.openclaw/openclaw.json | jq '.agents.defaults.model.primary'"
# Expected: "google-antigravity/claude-opus-4-5-thinking"

# 2. OAuth accounts
ssh netcup "docker exec $CONTAINER openclaw models status | grep 'google-antigravity:' | wc -l"
# Expected: 9

# 3. Skills
ssh netcup "docker exec $CONTAINER ls /root/openclaw-workspace/skills/ | wc -l"
# Expected: 3

# 4. Memory files
ssh netcup "docker exec $CONTAINER ls /root/openclaw-workspace/MEMORY.md"
# Expected: File exists

# 5. Workspace files
ssh netcup "docker exec $CONTAINER ls /root/openclaw-workspace/ | grep -E 'AGENTS|SOUL|TOOLS|USER|IDENTITY|HEARTBEAT'"
# Expected: All files present

# 6. Plugins
ssh netcup "docker exec $CONTAINER openclaw plugins list | grep telegram-enhanced"
# Expected: telegram-enhanced (enabled)
```
...
ssh netcup "docker volume create openclaw-home"
```
...
ssh netcup "docker exec $CONTAINER bash -c '
...
# Restart gateway
ssh netcup "docker exec $CONTAINER openclaw gateway restart"
```
...
```powershell
# Check volume mount
ssh netcup "docker inspect $CONTAINER | grep -A 5 'Mounts'"
...
ssh netcup "docker inspect $CONTAINER | grep openclaw-config"
