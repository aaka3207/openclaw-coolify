...
**How it works:**
1. User accesses https://bot.appautomation.cloud
...
**Check plugin status:**
```bash
ssh netcup "docker exec <container-name> openclaw plugins list"
```

**Get plugin info:**
```bash
ssh netcup "docker exec <container-name> openclaw plugins info telegram-enhanced"
```

**Enable/disable plugin:**
```bash
ssh netcup "docker exec <container-name> openclaw plugins enable telegram-enhanced"
ssh netcup "docker exec <container-name> openclaw plugins disable telegram-enhanced"
```
...
**Public URL:** https://bot.appautomation.cloud
...
2. **OpenClaw Token:**
   ...
   - Append to URL: `?token=5e9721970ba74e2c9ca3d854bee715b1b923c51dfb6a8942`

**Full URL with token:**
```
https://bot.appautomation.cloud?token=5e9721970ba74e2c9ca3d854bee715b1b923c51dfb6a8942
```
...
# Then open: http://localhost:18789?token=5e9721970ba74e2c9ca3d854bee715b1b923c51dfb6a8942
```
...
# Or via SSH
ssh netcup "docker exec <container-name> openclaw message send --channel telegram --target @user --message 'Test' --buttons '[[{\"text\":\"Yes\",\"callback_data\":\"yes\"}]]'"
```
...
- Example: `ssh netcup "sudo docker ps"` not `ssh netcup "docker ps"`
- Example: `ssh netcup "sudo /root/script.sh"` not `ssh netcup "/root/script.sh"`
...
# VPS
ssh netcup "docker exec <container-name> bash /app/scripts/test-telegram-buttons.sh @username"
```
...
**Check plugin status:**
```bash
ssh netcup "docker exec <container-name> openclaw plugins doctor"
```
...
**Check Models:**
```bash
ssh netcup "sudo docker exec <container-name> openclaw models status"
ssh netcup "sudo docker exec <container-name> openclaw models list"
```
...
```bash
ssh netcup "sudo docker ps --filter name=openclaw-qsw --format '{{.Names}}' | head -1"
# Output: openclaw-qsw0sgsgwcog4wg88g448sgs-234724397157
```

#### Step 2: Create Backup (MANDATORY)
```bash
# Backup in Docker volume (persists across container restarts)
ssh netcup "sudo cp /var/lib/docker/volumes/qsw0sgsgwcog4wg88g448sgs_openclaw-config/_data/openclaw.json /var/lib/docker/volumes/qsw0sgsgwcog4wg88g448sgs_openclaw-config/_data/openclaw.json.backup-DESCRIPTION"
...
```bash
ssh netcup "sudo ls -la /var/lib/docker/volumes/qsw0sgsgwcog4wg88g448sgs_openclaw-config/_data/*.backup*"
```
...
with open("/root/.openclaw/openclaw.json", "w") as f:
    json.dump(config, f, indent=2)
print("SUCCESS!")
'@ | ssh netcup "sudo docker exec -i <container-name> python3"

# Method 2: Create script file in container
ssh netcup "sudo docker exec <container-name> bash -c 'cat > /tmp/update.py << EOF
import json
# ... your script ...
EOF
...
ssh netcup "sudo docker exec <container-name> openclaw config get agents.defaults.heartbeat"
ssh netcup "sudo docker exec <container-name> openclaw config get agents.defaults.subagents"
ssh netcup "sudo docker exec <container-name> openclaw config get agents.defaults.imageModel"
ssh netcup "sudo docker exec <container-name> openclaw config get agents.defaults.models"

# Verify JSON is valid
ssh netcup "sudo docker exec <container-name> python3 -c 'import json; json.load(open(\"/root/.openclaw/openclaw.json\"))' && echo 'JSON is valid'"
```

#### Step 6: Restart Gateway to Apply
```bash
ssh netcup "sudo docker restart <container-name>"

# Wait for startup (30-45 seconds)
timeout /t 45

# Or check status until healthy
ssh netcup "sudo docker exec <container-name> openclaw status"
```
...
ssh netcup "sudo docker exec <container-name> openclaw status --deep"
```
...
ssh netcup "sudo docker exec <container-name> openclaw status" | grep "Heartbeat"

# Verify sessions are active
ssh netcup "sudo docker exec <container-name> openclaw status" | grep "Sessions"
```
...
ssh netcup "sudo cp /var/lib/docker/volumes/qsw0sgsgwcog4wg88g448sgs_openclaw-config/_data/openclaw.json.backup-DESCRIPTION /var/lib/docker/volumes/qsw0sgsgwcog4wg88g448sgs_openclaw-config/_data/openclaw.json"
...
with open("/root/.openclaw/openclaw.json", "w") as f:
    json.dump(config, f, indent=2)
print("SUCCESS: Task-based model configuration applied!")
'@ | ssh netcup "sudo docker exec -i openclaw-qsw0sgsgwcog4wg88g448sgs-234724397157 python3"
```
...
## Adding New Google Antigravity OAuth Accounts

Your setup uses **Google Antigravity** to access Claude models through multiple Google accounts for rate limit distribution. Currently you have **9 OAuth profiles** configured.

### Why Add More Accounts?

- **Higher rate limits** - Each account has its own quota
- **Better resilience** - If one account hits limits, others take over
- **Load distribution** - Requests spread across multiple accounts
- **Redundancy** - System keeps working if some accounts expire

### Current Accounts (9 total):
...
```
URL: https://bot.appautomation.cloud?token=xK7mR9pL2nQ4wF6jH8vB3cT5yG1dN0sA
```

**Step 2: Navigate to Auth Settings**
- Go to Settings → Models → Authentication
- Or use CLI: `openclaw models auth login --provider google-antigravity`

**Step 3: Start OAuth Flow**

**Method A: Via Dashboard (Recommended)**
1. Click "Add Account" or "Login with Google Antigravity"
2. Browser will open OAuth consent screen
3. Sign in with the NEW Google account
4. Grant permissions to Google Antigravity
5. Account will be added automatically

**Method B: Via SSH (if dashboard not accessible)**
```bash
# SSH into VPS
ssh netcup
...
# Start OAuth flow
sudo docker exec -it $CONTAINER openclaw models auth login --provider google-antigravity

# Follow the URL printed in terminal
# Sign in with NEW Google account
# Paste the code back when prompted
```

**Step 4: Verify Account Added**
```bash
...
# Example output:
# - google-antigravity:newaccount@gmail.com (newaccount@gmail.com) ok expires in 59m
```

**Step 5: Test New Account**
```bash
# Check quota usage
ssh netcup "sudo docker exec <container-name> openclaw models status" | grep "google-antigravity usage"

# Should show quota for all accounts including new one
```
...
```json
{
  "google-antigravity:newaccount@gmail.com": {
...
**DO NOT edit this file manually!** Use the OAuth flow to add accounts.

### Account Rotation Strategy

OpenClaw automatically rotates between accounts:
- Uses least-recently-used account first
- Skips accounts in cooldown (rate limited)
- Skips expired accounts (auto-refreshes if possible)
...
# Check quota usage
ssh netcup "sudo docker exec <container-name> openclaw models status" | grep "usage:"

# Example output:
# - google-antigravity usage: gemini-3-flash 100% left ⏱6d 23h
```
...
```bash
# List all profiles
ssh netcup "sudo docker exec <container-name> openclaw models auth list"

# Remove specific account
ssh netcup "sudo docker exec <container-name> openclaw models auth remove google-antigravity:account@gmail.com"
...
- Verify Google account has access to Antigravity
- Try incognito/private browser window
- Clear browser cookies for accounts.google.com

**Problem: Account shows "expired"**
- OpenClaw auto-refreshes tokens
- If refresh fails, re-authenticate:
  ```bash
  ssh netcup "sudo docker exec <container-name> openclaw models auth login --provider google-antigravity"
  ```

**Problem: Account not being used**
- Check if account is in cooldown (rate limited)
- Check if token is expired
- Verify account has quota remaining

**Problem: "No available auth profile"**
- All accounts are in cooldown or expired
- Add more accounts or wait for cooldown to expire
- Check `openclaw models status --json` for details

### Best Practices

1. **Add accounts gradually** - Test each one works before adding more
2. **Use different Google accounts** - Don't use same account twice
3. **Monitor quota usage** - Check regularly with `openclaw models status`
4. **Keep tokens fresh** - Re-authenticate if you see expiry warnings
5. **Document accounts** - Keep a list of which accounts you've added
6. **Test after adding** - Verify new account is being used

### Security Notes

- OAuth tokens are stored encrypted in auth-profiles.json
- Tokens are in Docker volume (persist across container restarts)
- Never share auth-profiles.json file (contains access tokens)
- Tokens auto-refresh, but may need re-auth after ~7 days
- Each account is independent (one compromise doesn't affect others)

### Recommended Account Count

- **Minimum:** 3-5 accounts (basic redundancy)
- **Recommended:** 9-12 accounts (good distribution)
- **Maximum:** No hard limit, but diminishing returns after 15-20

**Current setup: 9 accounts (good balance)**
